...
/* Check operation type */
    ...
    // return 0;
OperationType check_operation_type(char *argv[]); // encode and decode function
...
Status encode_byte_to_lsb(char data, char *image_buffer,int );

/* Copy remaining image bytes from src to stego image after encoding */
Status copy_remaining_img_data(FILE *fptr_src, FILE *fptr_dest);
/*start decoding bytes from stego image*/
// Status do_decoding(EncodeInfo *encInfo);
// /*open files start decoding*/
...
// Status decode_file_data(EncodeInfo *encInfo);
